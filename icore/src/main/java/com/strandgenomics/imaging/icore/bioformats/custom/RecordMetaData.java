...
/**
 * A collection of files and other relevant information as the source of a (potential) Record
 * @author arunabha
 * @author Yogi
 * @author anup
 */
public class RecordMetaData implements Serializable {
	 ...
	 */
	public RecordMetaData(File rootDirectory, RecordIdentifier id, Set<ImageIdentifier> images)
	{
	...
	private RecordMetaData(File rootDirectory, String recordLabel, TreeMap<Dimension, ImageIdentifier> imageDimnsions)
	{
		this.rootDirectory = rootDirectory.getAbsoluteFile();
		this.name = recordLabel;
		this.imageDimnsions = imageDimnsions;
	}
	 ...
	 * @throws IOException 
	 */
	public static final RecordMetaData load(File seedFile) throws IOException
	{
		...
		while( (line = br.readLine()) != null )
		{
			...
			if(fields[0].equals("isLittleEndian"))
			{
				isLittleEndian = Boolean.parseBoolean(fields[1]);
			}
			else if(fields[0].equals("NoOfFrames"))
				noOfFrames = Integer.parseInt(fields[1]);
			
			else if(fields[0].equals("NoOfChannels"))
				noOfChannels = Integer.parseInt(fields[1]);
			
			else if(fields[0].equals("NoOfSlices"))
				noOfSlices = Integer.parseInt(fields[1]);
			
			else if(fields[0].equals("NoOfSites"))
				noOfSites = Integer.parseInt(fields[1]);
			
			else if(fields[0].equals("MultiImageCoordinate"))
				multiImageCoordinate = FieldType.valueOf(fields[1]);
			
			else if(fields[0].equals("ChannelNames"))
			{
			...
			else if(fields[0].equals("SiteNames"))
			{
				String names[] = fields[1].split(",");
			...
			else if(fields[0].equals("PixelSizeAlongXAxis"))
				pixelSizeAlongXAxis = Double.parseDouble(fields[1]);
			
			else if(fields[0].equals("PixelSizeAlongYAxis"))
				pixelSizeAlongYAxis = Double.parseDouble(fields[1]);
			
			else if(fields[0].equals("PixelSizeAlongZAxis"))
				pixelSizeAlongZAxis = Double.parseDouble(fields[1]);
			
			else if(fields[0].equals("PixelDepth"))
			{
				int d = Integer.parseInt(fields[1].split(" ")[0])/8;
				pixelDepth = PixelDepth.toPixelDepth(d);
			}
			
			else if(fields[0].equals("ImageWidth"))
				imageWidth = Integer.parseInt(fields[1]);
			
			else if(fields[0].equals("ImageHeight"))
				imageHeight = Integer.parseInt(fields[1]);
			
			else if(fields[0].equals("ImageType"))
				imageType = ImageType.valueOf(fields[1]);
			
			else if(fields[0].equals("RecordLabel"))
				recordLabel = fields[1];
			
			else if(fields[0].equals("AcquiredTime"))
			{
				acquiredTime = new Date(Long.parseLong(fields[1]));
			}
			
			//the images for specific dimensions
			else if(fields[0].equals("Dimension"))
			{
				String dim[] = fields[1].split(",");
				
				int frame = Integer.parseInt(dim[0]);
				int slice = Integer.parseInt(dim[1]);
				int channel = Integer.parseInt(dim[2]);
				int site = Integer.parseInt(dim[3]);
				String filename = dim.length > 4 ? dim[4] : fields[2]; //for backward compatibility
				
				double positionX = dim.length > 5 ? Double.parseDouble(dim[5]) : 0.0;
				double positionY = dim.length > 6 ? Double.parseDouble(dim[6]) : 0.0;
				double positionZ = dim.length > 7 ? Double.parseDouble(dim[7]) : 0.0;
				double deltaTime = dim.length > 8 ? Double.parseDouble(dim[8]) : 0.0;
				double exposureTime = dim.length > 9 ? Double.parseDouble(dim[9]) : 0.0;
				
				//each file represents an image of a specific dimension
				Dimension d = new Dimension(frame, slice, channel, site);
				ImageIdentifier image = new ImageIdentifier(filename, recordLabel,
						""+d.frameNo, ""+d.sliceNo, ""+d.channelNo, ""+d.siteNo, 
						imageWidth, imageHeight, pixelDepth, imageType, isLittleEndian,
						pixelSizeAlongXAxis, pixelSizeAlongYAxis, pixelSizeAlongZAxis,
						positionX, positionY, positionZ, 
						deltaTime, exposureTime, acquiredTime, multiImageCoordinate);
				
				imageDimnsions.put(d, image);
			}
		}
		
		RecordMetaData rm = new RecordMetaData(rootDirectory, recordLabel, imageDimnsions);
