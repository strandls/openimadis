...
public final class OpenSlide implements Closeable {
    ...
    public OpenSlide(File file) throws IOException {
        ...
        // store level count
        levelCount = OpenSlideJNI.openslide_level_count(osr);
    ...
    private void checkDisposed() {
        if (osr == 0) {
            throw new RuntimeException("OpenSlide object has been disposed");
        }
    }
    ...
    public BufferedImage createImage(int level,int x,int y,int w,int h){
    	//long w = this.getLevelWidth(level);
		//long h = this.getLevelHeight(level);
    	x = x * (int) Math.pow(2, level);
    	y = y * (int) Math.pow(2, level);
        ...
        try {
			this.paintRegionARGB(data, x, y, level, img.getWidth(), img
			        .getHeight());
