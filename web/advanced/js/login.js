var login = Ext.create('Ext.form.Panel', {
					...
					items : [ {
						...
						xtype : 'radiogroup',
						fieldLabel : 'Choose Client',
						items : [ {
							boxLabel : 'Web Client',
							name : 'checkAdmin',
							inputValue : 'false',
							checked : !(document.URL.split("?")[1] == "admin"),
							style : 'color: #FFF; font-size: 14px;',
							labelStyle : 'color: #FFF; font-size: 14px;'
						}, {
							boxLabel : 'Admin Client',
							name : 'checkAdmin',
							inputValue : 'true',
							checked : (document.URL.split("?")[1] == "admin"),
							style : 'color: #FFF; font-size: 14px;'
						...
						if (form.isValid()) {
							Ext.Ajax.request({
								...
								params : {
									...
									checkAdmin : !form.findField('checkAdmin').getValue()
								},
								success : function (result, response) {
									if (result.responseText == "success") {
										if (!form.findField('checkAdmin').getValue()) {
//											window.location = "admin.jsp";
											window.location = location.href.substring(0, location.href.lastIndexOf("/")) + "?admin";
										} else {
