...
(function(Raphael) {
	...
	Raphael.sketchpad.VERSION = 0.5;
        ...
        Raphael.sketchpad.pen = function(paper, options) {
            return new Pen();            
        }
	...
	var Pen = function() {
		...
		self.start = function(e, sketchpad) {
			...
			self._points.push([e.pageX - offset.left, e.pageY - offset.top]);

			self._c = sketchpad.paper().path();

			self._c.attr({ 
				stroke: self._color,
				"stroke-opacity": self._opacity,
				"stroke-width": self._width,
		...
		self.move = function(e, sketchpad) {
			if (self._drawing == true) {
				...
				self._c.attr({ path: self.points_to_svg() });
			}
		};

		self.points_to_svg = function() {
			if (self._points != null && self._points.length > 1) {
				var p = self._points[0];
				var path = "M" + p[0] + "," + p[1];
				for (var i = 1, n = self._points.length; i < n; i++) {
					p = self._points[i];
					path += "L" + p[0] + "," + p[1]; 
				} 
				return path;
