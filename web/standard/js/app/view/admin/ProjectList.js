 ...
 */
Ext.define('Manage.view.admin.ProjectList', {
	...
	this.store = Ext.create('Manage.store.admin.Projects');
            ...
            columns : [
                ...
                {header : 'Storage Quota (in GB)', dataIndex : 'storageQuota', flex : 1, renderer : Ext.util.Format.numberRenderer("0.0")},
				{
					...
					renderer : function(value, metaData, record, rowIndex, colIndex, store, view) 
					{
						...
						var locationValue = '';
						var textValue = '';
						if(projectList.dirtyFlg == true)
							projectList.dirtyFlg = true;
						else
							projectList.dirtyFlg = false;
						...
						if(record.data.status == 'Active')
						{
							...
							textValue = 'Archive';
							disabled = false;
							projectList.dirtyFlg = projectList.dirtyFlg || false;
						}
						else if(record.data.status == 'Queued For Archiving')
						{
							...
							projectList.dirtyFlg = projectList.dirtyFlg || true;
							disabled = true;
						}
							...
							items : [
							         {
										...
										handler : function() 
										{
											projectList.fireEvent('onArchiveRestoreClick', record.data.name, locationValue, urlValue);
											projectList.doLayout(); // force buttons to redraw, since button values have changed
										}
							         }
									]
